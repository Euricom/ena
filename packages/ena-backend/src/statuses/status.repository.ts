...
export class StatusRepository extends Repository<Status> {
    ...
    findByType(type: StatusType, onlyIfLatest: boolean): Promise<Status[]> {
        ...
        if (onlyIfLatest) {
            query = query.innerJoinAndSelect(qb => {
                return qb.subQuery().from(Status, 'Status').select('MAX(Status.createdAt) as createdAt, Status.expense as expense').groupBy('Status.expense')
            }, 'joined', 'Status.expense = joined.expense and Status.createdAt = joined.createdAt')
        }
